...
/*---------------------------------------------------------------------------------------------------------*/
#define AUDIOBUFFERSIZE 320					//unit: PCM sample count
#define DPWMSAMPLERATE  16000			   	//unit:	sample count per second
//#define COMPBUFSIZE     AUDIOBUFFERSIZE/2	//uint: byte
...
extern sLibAdpcmState	sAdpcmState;
...
void Record2SPIFlash(uint32_t RecordStartAddr, uint32_t TotalPCMCount)
{
	...
	sflash_erase(&g_SPIFLASH, RecordStartAddr, (TotalPCMCount/2)+4095);	//Erase unit is 4096B
	...
	while (CallbackCounter <= (TotalPCMCount/AUDIOBUFFERSIZE))
	{
		if (RecordDataReady>=0)				   // Late to write flash if RecordDataReady is larger than 0
		{
		    ...
		    LibAdpcm_EncodeBlock( (PINT16)DataReadyAddr, s8Out_bytes, AUDIOBUFFERSIZE, &sAdpcmState);


			//Write ready buffer data 
			sflash_write(&g_SPIFLASH, FlashRecordAddr, (unsigned long *) s8Out_bytes, COMPBUFSIZE);



			FlashRecordAddr = FlashRecordAddr +  COMPBUFSIZE;
			RecordDataReady--;
...
void InitialADC(void)
{
	...
	*/
	...
	sParam.u8SDAdcDivisor = (256000/DPWMSAMPLERATE);		//OSR192 :32 for 8K, 16 for 16K
