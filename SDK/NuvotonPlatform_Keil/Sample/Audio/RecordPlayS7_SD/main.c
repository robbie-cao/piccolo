...
extern BOOL bPlaying, bRecording;
...
{						
	...
	Record2SD(0);			//Parameter is for the sector offset from record base SD_RECORD_START_SEC
	while(1)
	{
		if(bRecording==TRUE)				//Playing message handling
			RecordLoop();
		else
		{
			RecordClose();
	...
	PlaySdRecordG722(0);  	//Parameter is for the sector offset from record base SD_RECORD_START_SEC
	while(1)
	{
		if(bPlaying==TRUE)					//Playing message handling
			PlayLoop();
		else
		{
			PlayClose();
